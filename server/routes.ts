...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/projects", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log(`🔍 PROJECTS ACCESS: UserID: ${userId}, Role: ${user?.role}, IsAdmin: ${isAdmin}`);
      
      const projects = await storage.getProjects(isAdmin ? undefined : userId);
      
      console.log(`✅ PROJECTS LOADED: ${projects.length} projects for user ${userId}`);
      res.json(projects);
    } catch (error) {
  ...
  app.get("/api/projects/:id", isAuthenticated, async (req: any, res: any) => {
    try {
      ...
      console.log(`🔍 PROJECT ACCESS: ProjectID: ${projectId}, UserID: ${userId}, IsAdmin: ${isAdmin}`);
  ...
  app.post("/api/projects", isAuthenticated, async (req: any, res: any) => {
    try {
      ...
      if (!req.user || !req.user.id) {
        console.log(`🚫 PROJECT CREATE: No authenticated user found`);
      ...
      if (!user || (user.role !== 'admin' && user.role !== 'manager')) {
        console.log(`🚫 PROJECT CREATE: Access denied - UserID: ${userId}, Role: ${user?.role || 'unknown'}`);
        return res.status(403).json({ message: "Insufficient permissions" });
      }
      
      console.log(`✅ PROJECT CREATE: Access granted - UserID: ${userId}, Role: ${user.role}`);
      ...
      console.log(`✅ PROJECT CREATED: ProjectID: ${project.id}, UserID: ${userId}`);
  ...
  app.post("/api/companies", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (user?.role === "user") {
        console.log(`🚫 COMPANY CREATE: Access denied - UserID: ${userId}, Role: ${user?.role}`);
        return res.status(403).json({ message: "Insufficient permissions" });
      }
      ...
      console.log(`✅ COMPANY CREATED: CompanyID: ${company.id}, UserID: ${userId}`);
...
Diese E-Mail wurde automatisch generiert vom Bau-Structura Hochwasserschutz-System.
  ...
  app.put("/api/admin/users/:id/role", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      if (user.id === req.user.id) {
        return res.status(400).json({ message: "Sie können Ihre eigene Rolle nicht ändern" });
      }
      ...
      res.json({ 
        success: true, 
        message: `Rolle von ${user.email} zu ${role} geändert`,
        user: { 
          id: user.id, 
          email: user.email, 
          role: role,
      ...
      res.status(500).json({ message: "Rolle konnte nicht geändert werden" });
    }
  });
  ...
  app.post("/api/email/test-welcome", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      console.log(`🧪 Sende Test-Willkommens-E-Mail an: ${testEmail}`);
      ...
      res.json({ 
        ...
        timestamp: new Date().toISOString(),
        features: [
          '✅ Berechtigungen für User-Rolle erklärt',
          '🚫 Einschränkungen klar aufgezeigt',
          '💡 Upgrade-Hinweise integriert',
          '📂 SFTP-Account-Informationen',
